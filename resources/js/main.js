...
function add() {
  ...
  if (validationAll()) {
    ...
    if (DB.length == 0) {
      ...
      outer: for (var i = 0; i < DB.length; i++) {
        if (userName.value.toLowerCase().trim() == DB[i].name) {
          falg = false;
          break outer;
        }
      }
  ...
  if (DB.length != 0) {
    for (var i = 0; i < numberOfContacts; ++i) {
      ...
      temp +=
        ...
        i +
        ' \')" id="deleteContact" onmouseover="dHover(this.parentElement)" onmouseout="sHover(this.parentElement)" class="fas contact-icon fa-minus-circle"></i><i  class="fas contact-icon fa-edit"></i> <a href=tel:\'' +
        DB[i].phone +
